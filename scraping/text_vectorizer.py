#!/usr/bin/env python3
"""
Text Vectorizer - Processa JSONs do WebScraping e gera embeddings usando Google Gemini
"""
...
class TextVectorizer:
    ...
    def generate_embedding(self, text: str) -> List[float]:
        ...
        try:
            # Remove texto muito longo (máximo 2048 caracteres para evitar limite da API)
            if len(text) > 2048:
                text = text[:2048] + "..."
        ...
        for item in scraped_data:
            try:
                ...
                if status != 'success' or not content:
                    logger.warning(f"Pulando item de {site_name}: status={status}")
                    continue
                ...
                logger.error(f"Erro ao processar item de {item.get('site_name', 'N/A')}: {e}")
                self.error_count += 1
    ...
    def run(self):
        ...
        try:
            ...
            # Estatísticas finais
            logger.info(f"Vectorização concluída!")

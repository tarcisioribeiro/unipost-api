...
if GOOGLE_GEMINI_API_KEY:
    ...
    logger.warning(
        ...
        "vetorização automática desabilitada")


def generate_embedding_for_text(
        text_content: str, max_retries: int = 3,
        retry_delay: float = 2.0) -> Optional[List[float]]:
    """
    ...
    for attempt in range(max_retries):
        try:
            logger.info(
                f"Tentativa {attempt + 1}/{max_retries} para gerar embedding")
            ...
            logger.info(
                f"Embedding gerado com sucesso na tentativa {attempt + 1}")
            return result['embedding']

        except Exception as e:
            error_str = str(e)
            logger.warning(
                f"Tentativa {attempt + 1}/{max_retries} falhou: {error_str}")
            ...
            if ("429" in error_str or "quota" in error_str.lower() or
                ...
                if attempt < max_retries - 1:  # Não é a última tentativa
                    ...
                    logger.info(
                        f"Rate limit detectado. Aguardando {delay}s antes da "
                        f"próxima tentativa...")
                    time.sleep(delay)
                    continue
                else:
                ...
                logger.error(
                    f"Erro não recuperável ao gerar embedding: {error_str}")
                return None
...
def process_embedding_creation(text_instance):
    ...
    if not Embedding:
        logger.warning(
            "Modelo Embedding não disponível - embedding não processado")
        return False
    ...
    try:
        ...
        if existing_embedding:
            logger.info(
                f"Atualizando embedding existente para Text "
                f"{text_instance.id}")

            # Gera novo embedding
            embedding_vector = generate_embedding_for_text(
                text_instance.content)

            if not embedding_vector:
                logger.error(
                    f"Falha ao gerar embedding para atualização - "
                    f"Text {text_instance.id}")
                return False

            # Atualiza embedding existente
            existing_embedding.content = text_instance.content
            existing_embedding.title = (
                f"{text_instance.get_platform_display()}: "
                f"{text_instance.theme}")
            ...
            existing_embedding.metadata.update({
                ...
                "updated_at": (
                    text_instance.updated_at.isoformat()
                    if text_instance.updated_at else None),
                "last_embedding_update": time.time()
            })
            existing_embedding.save()

            logger.info(
                f"Embedding atualizado com sucesso: {existing_embedding.id}")
            return True
        ...
        if not embedding_vector:
            logger.error(
                f"Falha ao gerar embedding para novo texto - "
                f"Text {text_instance.id}")
            return False
        ...
        metadata = {
            ...
            "is_approved": text_instance.is_approved,
            "created_at": (
                text_instance.created_at.isoformat()
                if text_instance.created_at else None),
            "updated_at": (
                text_instance.updated_at.isoformat()
                if text_instance.updated_at else None),
            "auto_generated": True,
        ...
        logger.info(
            f"Embedding criado com sucesso: {embedding.id} para "
            f"Text {text_instance.id}")
        return True
...
def create_embedding_on_text_save(sender, instance, created, **kwargs):
    ...
    """
    # Verifica se deve processar - só gera embedding quando aprovado (não na criação)
    should_process = not created and instance.is_approved

    if not should_process:
        logger.debug(
            f"Text {instance.id} não processado - created={created}, "
            f"approved={instance.is_approved}")
        return

    # Verifica se API está configurada
    if not GOOGLE_GEMINI_API_KEY:
        logger.warning(
            f"Embedding não processado para Text {instance.id} - "
            f"API não configurada")
        return

    logger.info(
        f"Processando embedding para Text {instance.id} "
        f"(created={created})")
    ...
    if not success:
        logger.warning(
            f"Falha ao processar embedding para Text {instance.id} - "
            f"será tentado novamente na próxima atualização")
